...
def load_activation_mean(filename):
    ...
    if not os.path.exists(found_path):
        raise FileNotFoundError(
            ...
            f"Make sure extract_activations_gemma.py has been run and files are in {ACTIVATIONS_DIR}/"
        )
...
def find_universal_neurons_3sigma(delta_math, delta_geo):
    ...
    DIRECTLY to model.model.language_model.layers[layer_idx] — no offset needed.
    """
    # Compute per-array thresholds
    threshold_math = 2.4 * np.std(delta_math)
    threshold_geo  = 2.4 * np.std(delta_geo)
...
def main():
    print("=" * 60)
    print("EXTRACTING UNIVERSAL MLP NEURONS — GEMMA 3 (3-Sigma Cross-Domain)")
    print("=" * 60)
