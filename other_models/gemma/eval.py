        ...
        [
            {
                ...
                "content": [
                    {
                        "type": "text", 
                        "text": "You are a participant in this experiment and must engage thoughtfully. You strictly follow rules and always output in the exact requested format.\n\n" + prompt
                    }
                ]
            }
        ],
...
def main():
    ...
    print("\n" + "="*62)
    print("  RESULTS  (4 folds × 24 rows)  , file: neurons_all.json")
    print("="*62)
    ...
    with open("results/asdiv_results.json", "w") as f:
        ...
        paired_tests_dict = {
            "avg_pts_fold_level":  {"t": float(t_fold_pts) if not np.isnan(t_fold_pts) else 0.0, "p": float(p_fold_pts) if not np.isnan(p_fold_pts) else 1.0, "n": 4, "sig": stars(p_fold_pts)},
            "avg_pts_item_level":  {"t": float(t_item_pts) if not np.isnan(t_item_pts) else 0.0, "p": float(p_item_pts) if not np.isnan(p_item_pts) else 1.0, "n": len(base_item_pts), "sig": stars(p_item_pts)},
            "optimal_fold_level":  {"t": float(t_fold_opt) if not np.isnan(t_fold_opt) else 0.0, "p": float(p_fold_opt) if not np.isnan(p_fold_opt) else 1.0, "n": 4, "sig": stars(p_fold_opt)},
            "optimal_item_level":  {"t": float(t_item_opt) if not np.isnan(t_item_opt) else 0.0, "p": float(p_item_opt) if not np.isnan(p_item_opt) else 1.0, "n": len(base_item_opt), "sig": stars(p_item_opt)},
        }
