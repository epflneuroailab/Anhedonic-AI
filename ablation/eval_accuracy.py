...
# ── Folds: same split as eval_model_a.py ─────────────────────────────────────
...
def run(folds, label):
    ...
    for fi, fold in enumerate(folds):
        ...
        print(f"    → accuracy={acc:.2%}  "
              + "  ".join(f"{p}pt={np.mean(v):.2%}" for p, v in sorted(pts_correct.items())))
...
with open("results/accuracy_results.json", "w") as f:
    json.dump({"baseline": {"folds": base_stats, "rows": base_rows},
               "model_a":  {"folds": modA_stats, "rows": modA_rows}}, f, indent=2)
